...
- 本地契约和原生 Haystack/Qdrant local 流水线、并发、来源别名与评测防泄漏测试 24 项通过；客户端断开后，未完成的问答仍占用并发名额。
- 已创建 https://github.com/chaos-lsc/haystack fork，当前 v2 checkout 的 origin 指向该 fork，upstream 指向 deepset-ai/haystack。

## 本次会话已收尾

- 已完成补齐语料后的 B0 开发集 208 题：109 正确（52.40%），65 错误作答、21 拒答、13 运行错误；P95 34.99 秒，Windows 进程 RSS 采样峰值约 14.00 GiB。完整结果见 [baseline-results.md](baseline-results.md)。
- 已按用户新边界停止：排队的 B1–B4 被拦截，保留测试集未运行、最终候选未冻结、未部署。本地实验 Qdrant 已关闭。后续开发须先重新明确边界，见 [session-boundary.md](session-boundary.md)。
- 开发集诊断发现旧索引遗漏 32 份 `.doc`；已使用本机 Word 只读导出，建立 `.lab-data-complete` 新快照，并保留母文件来源别名。原 B0 完整运行和 B1 部分运行仅保留作诊断；新快照目前只完成 B0。
...
- 五类 B4 开发题均能完成调用。诊断中发现引用排版差异和减法操作数方向问题：引用匹配容忍 Office 空白但不合并独立数字，表格规划提示明确变化量与增长率的槽顺序。校验仍可能拒绝选项正确但引用/计算过程不一致的回答，实际收益交由完整开发集判断。
- baseline 仅在本地评测。远端最终改进版的部署已转入后续待明确范围，本次不执行。

## 前置条件与未完成项

- 应用源码已推送到 chaos-lsc/haystack 的 trust-rag/reconstruction 分支；本轮 baseline 使用提交 7b79996。其他完整阶段评测已暂停。
...
- 评测异常分支对仅带工作表标注的失败题漏计定位分母。本轮报告已审计修正为 65/70，原始数据保留；运行时代码修复属于后续待明确范围。
